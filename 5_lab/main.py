...
if __name__ == '__main__':
    ...
    for i in range(1, len(length_list)):
        ...
        temporary_solution_1.append(get_length(w, abs(length_list[i] - solutions[i - 1][0]["l"])))
        temporary_solution_1.append(get_length(w, abs(0 - solutions[i - 1][0]["l"])))

        if temporary_solution_1[0] > temporary_solution_1[1]:
            final_solution_1 = {"l": length_list[i],
                                "prev_dist": temporary_solution_1[0] + solutions[i - 1][0]["prev_dist"]}
        else:
            final_solution_1 = {"l": 0,
                                "prev_dist": temporary_solution_1[1] + solutions[i - 1][0]["prev_dist"]}

        temporary_solution_2.append(get_length(w, abs(length_list[i] - solutions[i - 1][1]["l"])))
        temporary_solution_2.append(get_length(w, abs(0 - solutions[i - 1][1]["l"])))

        if temporary_solution_2[0] > temporary_solution_2[1]:
            final_solution_2 = {"l": length_list[i],
                                "prev_dist": temporary_solution_2[0] + solutions[i - 1][1]["prev_dist"]}
        else:
            final_solution_2 = {"l": 0,
                                "prev_dist": temporary_solution_2[1] + solutions[i - 1][1]["prev_dist"]}
